...
Graphic_Lib::Graphic_Lib() :  g_pD3D(NULL), g_pd3dDevice(NULL), backGroundTexture(NULL)
{
}

Graphic_Lib::Graphic_Lib(LPCWSTR windowName, int posX, int posY, int width, int height) :  g_pD3D(NULL), g_pd3dDevice(NULL), backGroundTexture(NULL)
{
...
void Graphic_Lib::InitWin(LPCWSTR windowName, int posX, int posY, int width, int height)
{
	// Register the window class
	WNDCLASSEX g_wc = { sizeof(WNDCLASSEX), CS_CLASSDC, MsgProc, 0L, 0L,
		GetModuleHandle(NULL), NULL, NULL, NULL, NULL,
...
void Graphic_Lib::LoadTexture(LPCWSTR img)
{
...
void Graphic_Lib::Initilize(void)
{
	InitWin();
	InitDX();
	//LoadBackGroundData();
...
void Graphic_Lib::ShowTheWindow()
{
...
//-----------------------------------------------------------------------------
LRESULT WINAPI Graphic_Lib::MsgProc( HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam )
{
    switch( msg )
    {
        case WM_DESTROY:
...
WNDCLASSEX Graphic_Lib::GetValueGWC()
{
	return g_wc;
};

void Graphic_Lib::PlayTheMusic()
{
	audio.Play();
}

void Graphic_Lib::LoadTheMusic()
{
